...
反馈是**下一顿补问上一顿**的，因此 `rated` 事件通常比它所评价的那顿晚一天甚至跨天。事件靠 `targetTs` 显式回指自己属于哪条观察值；没有该字段的历史事件与导入的旧快照退回启发式（取同 slot 同 dish 中时间上最近的组）。**改归约逻辑前先读 `2026-08-26-browse-and-implicit-signals-design.md` 的 §6.2**（改写了 `2026-08-22` 那份的同名节）—— 这里踩过一次坑。
